...
def irm_undirected(X, T, a, b, A, set_seed = True, random_seed = 42, print_iter = False):
    ...
    for t in range(T): # for T iterations
        ...
        for n in range(N): # for each node n
            ...
            likelihood = np.sum(betaln(M1+R+a, M0+M-R+b) - betaln(M1+a, M0+b), 1)
            # lik of current node to new component
            likelihood_n = np.sum(betaln(r+a, m-r+b) - betaln(a,b), 1)
...
Z = irm_undirected(X, T, a, b, A)

def irm_undirected_old(X, T, a, b, A, set_seed = True, random_seed = 42, print_iter = False):
    N = len(X)
    z = np.ones([N,1])
    Z = []
...
Z2 = irm_undirected_old(X,T,a,b,A)